...
n1=MAP_classificatino(img_94,img_94_1,m,v,pw);
text="MAP_accuracy percentage of img_94: " + num2str(n1);
disp(text)
subplot(1,2,2)
imshow(img_94_1)
...
n2=MAP_classificatino(img_86,img_86_1,m,v,pw);
text="MAP_accuracy percentage of img_86: " + num2str(n2);
disp(text)
subplot(1,2,2)
imshow(img_86_1)
title('given lable image')
...
n3=MAP_classificatino(img_99,img_99_1,m,v,pw);
text="MAP_accuracy percentage of img_99: " + num2str(n3);
disp(text)
subplot(1,2,2)
imshow(img_99_1)
title('given lable image')
...
n4=MAP_classificatino(img_105,img_105_1,m,v,pw);
text="MAP_accuracy percentage of img_105: " + num2str(n4);
disp(text)
subplot(1,2,2)
imshow(img_105_1)
title('given lable image')
...
%ML estimate
function out=MAP_classificatino(img_94,img_94_1,m,v,pw)
m1=m(1);
m2=m(2);
m3=m(3);
v1=v(1);
...
for i=1:h
    for j=1:w
        if k2(i,j)~=0
        px_w1= pw1*((1/sqrt(2*pi*v1))*exp((-1/(2*v1))*(k1(i,j)-m1)^2));
        px_w2= pw2*((1/sqrt(2*pi*v2))*exp((-1/(2*v2))*(k1(i,j)-m2)^2));
        px_w3= pw3*((1/sqrt(2*pi*v3))*exp((-1/(2*v3))*(k1(i,j)-m3)^2));
